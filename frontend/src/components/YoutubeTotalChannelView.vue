<template>
  <div id="youtube-total-channel-view-chart">
    <div class="toolbar">
      <v-row justify="end">
        <v-col cols="12">
          ...
          <v-btn
              ...
              elevation="1"
              class="mr-3"
              :disabled="this.three_months.length < 0"
        ...
        <v-col cols="12">
          ...
          <div class="month_change_percentage">
            <div class="stat">
              <span class="up"
                    v-if="(((this.latest_channel_view_count - this.past_month_channel_view_count)/this.past_month_channel_view_count)*100)>0">
                    +{{Number((((this.latest_channel_view_count - this.past_month_channel_view_count) / this.past_month_channel_view_count) * 100).toFixed(2)).toLocaleString() }}%
              </span>
              <span class="down"
                    v-if="(((this.latest_channel_view_count - this.past_month_channel_view_count)/this.past_month_channel_view_count)*100)<0">
                    {{Number((((this.latest_channel_view_count - this.past_month_channel_view_count) / this.past_month_channel_view_count) * 100).toFixed(2)).toLocaleString() }}%
...
export default {
  ...
  data() {
    return {
      ...
      selection: 'one_month',
      latest_date: '',
      latest_channel_view_count: '',
      past_month_channel_view_count: '',
      index_number: '',
      three_months: '',
      six_months: '',
      one_year: '',
  ...
  methods: {
    ...
    this.index_number = data["result"][data["result"].length - 1]["total_channel_view"]
    // console.log(this.youtube_videos)

      let formattedData = this.youtube_videos.map((e, i) => {
        return {
          x: e.datetime,
          y: e.total_channel_view,
        };
      });
      let formattedData1 = this.youtube_videos.map((e, i) => {
        return {
          x: e.datetime,
          y: e.channel_view_difference,
        };
      });
    ...
    update_data(timeline) {
      ...
      this.axios.get("http://localhost/api/youtube/stats/channel", {setTimeout: 10000})
          .then(res => {
            ...
            this.latest_channel_view_count = this.data[this.data.length - 1]["total_channel_view"]
            this.past_month_channel_view_count = this.data[this.data.length - 30]["total_channel_view"]
  ...
  computed: {
    oneMonthisDisable() {
      return this.one_month.length < 0;
    },
    threeMonthisDisable() {
      return this.three_months.length < 0;
    },
    sixMonthisDisable() {
      return this.six_months.length < 0;
    },
    oneYearisDisable() {
      return this.one_year.length < 0;
    }
  }
}
</script>
