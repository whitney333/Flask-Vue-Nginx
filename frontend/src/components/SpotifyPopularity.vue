<template>
  <div id="spotify-popularity-chart">
    <div class="toolbar">
      <v-row justify="end">
        <v-col cols="12">
        ...
        <v-col cols="12">
          ...
          <div class="month_change_percentage">
            <div class="stat">
              <span class="up"
                    v-if="(((this.latest_popularity_count - this.past_month_popularity_count)/this.past_month_popularity_count)*100)>0">
                    +{{
                  Number((((this.latest_popularity_count - this.past_month_popularity_count) / this.past_month_popularity_count) * 100).toFixed(2)).toLocaleString()
                }}%
              </span>
              <span class="down"
                    v-if="(((this.latest_popularity_count - this.past_month_popularity_count)/this.past_month_popularity_count)*100)<0">
...
export default {
  ...
  data() {
    return {
      ...
      latest_popularity_count: "",
      past_month_popularity_count: "",
      index_number: "",
      latest_date: "",
      chartOptions: {
        chart: {
  ...
  methods: {
    async get_spotify_popularity() {
      const {data} = await this.axios.get("/api/spotify/index?" + "end=" + this.end + "&range=" + this.range,
          {setTimeout: 10000})

      this.range = data["range"]
      this.posts = data["posts"]
      this.index_number = data["posts"][data["posts"].length - 1]["popularity"]
      // console.log(this.follower)

      let formattedData = this.posts.map((e, i) => {
        return {
          x: e.date,
          y: e.popularity,
        };
