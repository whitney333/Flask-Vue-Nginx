<template>
  <div id="bilibili-total-danmu-chart">
    <div class="toolbar">
      <v-row justify="end">
        <v-col cols="12">
      ...
      <v-row justify="end">
        <v-col cols="12">
          ...
          <div class="month_change_percentage">
            <div class="stat">
              <span class="up"
                    v-if="(((this.latest_danmu_count - this.past_month_danmu_count)/this.past_month_danmu_count)*100)>0">
                    +{{Number((((this.latest_danmu_count - this.past_month_danmu_count) / this.past_month_danmu_count) * 100).toFixed(2)).toLocaleString() }}%
              </span>
              <span class="down"
                    v-if="(((this.latest_danmu_count - this.past_month_danmu_count)/this.past_month_danmu_count)*100)<0">
                    {{Number((((this.latest_danmu_count - this.past_month_danmu_count) / this.past_month_danmu_count) * 100).toFixed(2)).toLocaleString() }}%
...
export default {
  ...
  data() {
    return {
      ...
      latest_date: "",
      latest_danmu_count: "",
      past_month_danmu_count: "",
      index_number: "",
      chartOptions: {
        chart: {
          id: 'bilibili-total-danmu',
          height: '100%',
          width: '100%',
          type: 'line',
          group: 'bilibili',
  ...
  methods: {
    async get_total_danmu() {
      const {data} = await this.axios.get("/api/bilibili/chart", {setTimeout: 10000})

      this.result = data["result"]
      this.index_number = data["result"][data["result"].length - 1]["total_danmu"]
    ...
    update_data(timeline) {
      ...
      this.axios.get("/api/bilibili/chart", {setTimeout: 10000})
          .then(res => {
            ...
            this.latest_danmu_count = this.data[this.data.length - 1]["total_danmu"]
            this.past_month_danmu_count = this.data[this.data.length - 6]["total_danmu"]
