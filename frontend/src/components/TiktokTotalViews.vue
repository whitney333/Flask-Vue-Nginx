<template>
  <div id="tiktok-total-view-chart">
    <div class="toolbar">
      <v-row justify="end">
        <v-col cols="12">
        ...
        <v-col cols="12">
          ...
          <div class="month_change_percentage">
            <div class="stat">
              <span class="up"
                    v-if="(((this.latest_view_count - this.past_month_view_count)/this.past_month_view_count)*100)>0">
                    +{{Number((((this.latest_view_count - this.past_month_view_count) / this.past_month_view_count) * 100).toFixed(2)).toLocaleString() }}%
              </span>
              <span class="down"
                    v-if="(((this.latest_view_count - this.past_month_view_count)/this.past_month_view_count)*100)<0">
                    {{Number((((this.latest_view_count - this.past_month_view_count) / this.past_month_view_count) * 100).toFixed(2)).toLocaleString() }}%
...
export default {
  ...
  data() {
    return {
      ...
      latest_date: '',
      latest_view_count: '',
      past_month_view_count: '',
      index_number: '',
      chartOptions: {
        chart: {
          id: 'tiktok-total-view',
          height: '100%',
          width: '100%',
          type: 'line',
          group: 'tiktok',
  ...
  methods: {
    async get_tiktok_total_views() {
      const {data} = await this.axios.get("/api/tiktok/chart", {setTimeout: 10000})
      this.tiktok_total_views = data["result"]
      this.index_number = data["result"][data["result"].length - 1]["total_views"]
      // console.log(this.tiktok_total_views)

      let formattedData = this.tiktok_total_views.map((e, i) => {
        return {
          x: e._id,
          y: e.total_views,
        };
      });
      let formattedData1 = this.tiktok_total_views.map((e, i) => {
        return {
          x: e._id,
          y: e.avg_view,
        };
    ...
    update_data(timeline) {
      ...
      this.axios.get("/api/tiktok/chart", {setTimeout: 10000})
          .then(res => {
            ...
            this.latest_view_count = this.data[this.data.length - 1]["total_views"]
            this.past_month_view_count = this.data[this.data.length - 30]["total_views"]
