<template>
  <div id="instagram-total-post-chart">
    <div class="toolbar">
      <v-row justify="end">
        <v-col cols="12">
        ...
        <v-col cols="12">
          ...
          <div class="month_change_percentage">
            <div class="stat">
              <span class="up"
                    v-if="(((this.latest_post_count - this.past_month_post_count)/this.past_month_post_count)*100)>0">
                    +{{
                  Number((((this.latest_post_count - this.past_month_post_count) / this.past_month_post_count) * 100).toFixed(2)).toLocaleString()
                }}%
              </span>
              <span class="down"
                    v-if="(((this.latest_post_count - this.past_month_post_count)/this.past_month_post_count)*100)<0">
...
export default {
  ...
  data() {
    return {
      ...
      latest_date: "",
      latest_post_count: "",
      past_month_post_count: "",
      index_number: "",
      chartOptions: {
        chart: {
          id: 'instagram-total-post',
          height: '100%',
          width: '100%',
          type: 'line',
          group: 'instagram',
  ...
  methods: {
    async get_instagram_post() {
      ...
      let formattedData = this.instagram_posts.map((e, i) => {
        return {
          x: e.datetime,
          y: e.media_count,
        };
    ...
    update_data(timeline) {
      ...
      this.axios.get("/api/instagram/chart/follower", {setTimeout: 10000})
          .then(res => {
            ...
            this.latest_post_count = this.data[this.data.length - 1]["media_count"]
            this.past_month_post_count = this.data[this.data.length - 30]["media_count"]
