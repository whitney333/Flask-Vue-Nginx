<template>
  <div id="tiktok-total-like-chart">
    <div class="toolbar">
      <v-row justify="end">
        <v-col cols="12">
        ...
        <v-col cols="12">
          ...
          <div class="month_change_percentage">
            <div class="stat">
              <span class="up"
                    v-if="(((this.latest_like_count - this.past_month_like_count)/this.past_month_like_count)*100)>0">
                    +{{Number((((this.latest_like_count - this.past_month_like_count) / this.past_month_like_count) * 100).toFixed(2)).toLocaleString() }}%
              </span>
              <span class="down"
                    v-if="(((this.latest_like_count - this.past_month_like_count)/this.past_month_like_count)*100)<0">
                    {{Number((((this.latest_like_count - this.past_month_like_count) / this.past_month_like_count) * 100).toFixed(2)).toLocaleString() }}%
...
export default {
  ...
  data() {
    return {
      ...
      latest_date: '',
      latest_like_count: '',
      past_month_like_count: '',
      index_number: '',
      chartOptions: {
        chart: {
          id: 'tiktok-total-like',
          height: '100%',
          width: '100%',
          type: 'line',
          // group: 'tiktok',
  ...
  methods: {
    async get_tiktok_total_likes() {
      const {data} = await this.axios.get("http://localhost/api/tiktok/chart", {setTimeout: 10000})
      this.tiktok_total_likes = data["result"]
      this.index_number = data["result"][data["result"].length - 1]["total_like"]
      // console.log(this.tiktok_total_likes)

      let formattedData = this.tiktok_total_likes.map((e, i) => {
        return {
          x: e.datetime,
          y: e.total_like,
        };
      });
      let formattedData1 = this.tiktok_total_likes.map((e, i) => {
        return {
          x: e.datetime,
          y: e.avg_like,
        };
    ...
    update_data(timeline) {
      ...
      this.axios.get("http://localhost/api/tiktok/chart/follower", {setTimeout: 10000})
          .then(res => {
            ...
            this.latest_like_count = this.data[this.data.length - 1]["tiktok_like"]
            this.past_month_like_count = this.data[this.data.length - 30]["tiktok_like"]
