<template>
  <div id="youtube-total-video-comment-chart">
    <div class="toolbar">
      <v-row justify="end">
        <v-col cols="12">
        ...
        <v-col cols="12">
          ...
          <div class="month_change_percentage">
            <div class="stat">
              <span class="up"
                    v-if="(((this.latest_comment_count - this.past_month_comment_count)/this.past_month_comment_count)*100)>0">
                    +{{Number((((this.latest_comment_count - this.past_month_comment_count) / this.past_month_comment_count) * 100).toFixed(2)).toLocaleString() }}%
              </span>
              <span class="down"
                    v-if="(((this.latest_comment_count - this.past_month_comment_count)/this.past_month_comment_count)*100)<0">
                    {{Number((((this.latest_comment_count - this.past_month_comment_count) / this.past_month_comment_count) * 100).toFixed(2)).toLocaleString() }}%
...
export default {
  ...
  data() {
    return {
      ...
      past_month_comment_count: '',
      index_number: '',
      chartOptions: {
        chart: {
          id: 'youtube-total-video-comment',
          height: '100%',
          width: '100%',
          type: 'line',
          group: 'youtube',
  ...
  methods: {
    async get_youtube_video_comment() {
    const {data} = await this.axios.get("/api/youtube/stats/video", {setTimeout: 10000})
    this.youtube_videos = data["result"]
    this.index_number = data["result"][data["result"].length - 1]["total_comment"]
    ...
    update_data(timeline) {
      ...
      this.axios.get("/api/youtube/stats/video", {setTimeout: 10000})
          .then(res => {
            ...
            this.past_month_comment_count = this.data[this.data.length - 30]["total_comment"]
  ...
  created() {
    this.get_youtube_video_comment();
    this.update_data();
  },
  filters: {
