<template>
  <div id="tiktok-total-share-chart">
    <div class="toolbar">
      <v-row justify="end">
        <v-col cols="12">
        ...
        <v-col cols="12">
          ...
          <div class="month_change_percentage">
            <div class="stat">
              <span class="up"
                    v-if="(((this.latest_share_count - this.past_month_share_count)/this.past_month_share_count)*100)>0">
                    +{{Number((((this.latest_share_count - this.past_month_share_count) / this.past_month_share_count) * 100).toFixed(2)).toLocaleString() }}%
              </span>
              <span class="down"
                    v-if="(((this.latest_share_count - this.past_month_share_count)/this.past_month_share_count)*100)<0">
                    {{Number((((this.latest_share_count - this.past_month_share_count) / this.past_month_share_count) * 100).toFixed(2)).toLocaleString() }}%
...
export default {
  ...
  data() {
    return {
      ...
      latest_date: '',
      latest_share_count: '',
      past_month_share_count: '',
      chartOptions: {
        chart: {
          id: 'tiktok-total-share',
          height: '100%',
          width: '100%',
          type: 'line',
          group: 'tiktok',
  ...
  methods: {
    async get_tiktok_total_shares() {
      const {data} = await this.axios.get("http://localhost/api/tiktok/chart", {setTimeout: 10000})
      this.tiktok_total_shares = data["result"]
      this.index_number = data["result"][data["result"].length - 1]["total_share"]
      // console.log(this.tiktok_total_views)

      let formattedData = this.tiktok_total_shares.map((e, i) => {
        return {
          x: e._id,
          y: e.total_share,
        };
      });
      let formattedData1 = this.tiktok_total_shares.map((e, i) => {
        return {
          x: e._id,
          y: e.avg_share,
        };
    ...
    update_data(timeline) {
      ...
      this.axios.get("http://localhost/api/tiktok/chart", {setTimeout: 10000})
          .then(res => {
            ...
            this.latest_share_count = this.data[this.data.length - 1]["total_share"]
            this.past_month_share_count = this.data[this.data.length - 6]["total_share"]
