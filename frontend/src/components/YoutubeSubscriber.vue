      ...
      <v-row justify="end">
        ...
        <v-col cols="12">
          ...
          <div class="month_change_percentage">
            <div class="stat">
              <span class="up"
                    v-if="(((this.latest_subscriber_count - this.past_month_subscriber_count)/this.past_month_subscriber_count)*100)>0">
                    +{{Number((((this.latest_subscriber_count - this.past_month_subscriber_count) / this.past_month_subscriber_count) * 100).toFixed(2)).toLocaleString() }}%
              </span>
              <span class="down"
                    v-if="(((this.latest_subscriber_count - this.past_month_subscriber_count)/this.past_month_subscriber_count)*100)<0">
                    {{Number((((this.latest_subscriber_count - this.past_month_subscriber_count) / this.past_month_subscriber_count) * 100).toFixed(2)).toLocaleString() }}%
    ...
    update_data(timeline) {
      ...
      this.axios.get("http://localhost/api/youtube/stats/channel", {setTimeout: 10000})
          .then(res => {
            ...
            this.latest_subscriber_count = this.data[this.data.length - 1]["subscriber"]
            this.past_month_subscriber_count = this.data[this.data.length - 30]["subscriber"]
